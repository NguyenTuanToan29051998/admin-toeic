...
import {collectionService, examService} from '../../services';
...
const ExamQuestions: NextPage<Props> = ({
  exam,
  searchStr,
  sortDirection,
  pageNo,
  ...
  //     id: 'MD0001',
  //     img: 'img',
  //     name: 'Đề Thi thử TOEIC Online  2022',
  //     examType: 'ielts',
  //     examTime: '45:00',
  //     status: 'Ready',
  //   },
          ...
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
              ...
              <tbody>
                {(exam.data || []).map(item => (
                  <tr
                    ...
                    <td className="px-6 py-4">{`Toeic`}</td>
            ...
            <div className="flex justify-end my-5 mr-3">
              ...
              <PaginationSection
                ...
                pageCount={exam.totalPages}
                currentPage={exam.pageNo}
                setCurrentPage={async (number): Promise<void> => {
                  await router.push(
                    `/exams?searchStr=${searchStr}&sortDirection=${sortDirection}&pageNo=${number}&pageSize=${pageSize}`,
                  );
                }}
              />
            </div>
                  ...
                  <div className="mt-5 py-5 px-6">
                    ...
                    <div className="gap-4 items-center my-6">
                      ...
                      <label
                        ...
                        <div className="mt-2 px-5 py-8 w-[104px] border border-dashed border-blue-1890FF rounded-sm text-center hover:cursor-pointer">
                          {plusIconBS('text-2xl my-0 mx-auto')}
...
ExamQuestions.getInitialProps = async ({query, res}): Promise<Props> => {
  const {searchStr, sortDirection, pageNo, pageSize} = query;

  let exam = {} as ExamList;

  try {
    exam = await examService.getExams(
      searchStr?.toString() || '',
...
export default tailwindOnly(ExamQuestions);
